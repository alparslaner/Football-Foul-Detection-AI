...
class MVNetwork(torch.nn.Module):
    ...
    def extract_features(self, mvimages):
        """
        Extract the 7 features from the model without any classification.
        Returns a tensor of shape [batch_size, 7] containing the features.
        """
        with torch.no_grad():
            features = self.mvnetwork(mvimages)
        return features
